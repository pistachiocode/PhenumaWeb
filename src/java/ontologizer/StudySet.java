...
public class StudySet implements Iterable<ByteString>
{
	...
	/** Label used in the matrix */
	private ByteString[] labels;
...
//	}
	...
	public ByteString[] getLabels() {
		return labels;
	}


	public void setLabels(ByteString[] labels) {
		this.labels = labels;
	}
	...
	{   
            String str = "";
            ByteString[] genes = this.getAllElements();
            for(int i=0; i<genes.length ; i++){
                str = str +" "+ genes[i].toString();
            }
            
            return str;
            
            
	/*	String str = name + " (n=" + (getSize()) + ")";
		return str;*/
	}
	 ...
	 * @return
	 */
	public ByteString [] getAllElements()
	{
	 ...
	 * be called again.
	 */
	public void addElement(ByteString geneName, String description)
	{
	 ...
	 * @param attribute
	 */
	public void addElement(ByteString geneName, ItemAttribute attribute)
	{
	 ...
	 * @return
	 */
	public HashSet<ByteString> getAllNodesNames()
	{
	...
	public void filterOutDuplicateGenes(AssociationContainer associationContainer)
	{
		...
		{
			PP2Associations gene2Association = associationContainer.get(geneName);
	...
	public synchronized GOTermEnumerator enumerateGOTerms(Ontology graph, AssociationContainer associationContainer, Set<ByteString> evidences)
	{
		...
		for (ByteString geneName : gene2Attribute.keySet())
		{
			PP2Associations geneAssociations = associationContainer.get(geneName);
	...
	public static StudySet [] generateShuffledStudySets(StudySet [] studySetArray)
	{
		...
		for (i=0,j=0;i<studySetArray.length;i++)
		{
			...
			for (k=0;k<studySetArray[i].getSize();k++,j++)
				newStudySet.addElement(genesArray[j],descriptionArray[j]);
			array[i] = newStudySet; 
		}
	...
	public void writeSetWithAnnotations(Ontology graph, AssociationContainer associations, File file)
	{
		try
		{
			...
			for (ByteString gene : this)
			{
				...
				PP2Associations geneAssociations = associations.get(gene);
				if (geneAssociations != null)
				{
...
//						val += ",comment="+entry.getValue();
	...
	public void addGenes(StudySet studySet)
	{
		for (ByteString g : studySet)
			addElement(g, studySet.getItemAttribute(g));
	}
}
