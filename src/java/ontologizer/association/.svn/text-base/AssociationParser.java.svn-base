...
public class AssociationParser
{
	...
	/** ADDED key: dbReference (ej. OMIN:0000251), value: main gene name (dbObject_Symbol) */
...
//					assoc.setGoTermName(currentTermName);
				...
				/*	ArrayList<Association> gassociations = gene2Associations.get(assoc.getObjectSymbol());
					...
					ArrayList<Association> gassociations = gene2Associations.get(assoc.getDB_Object());
					if (gassociations == null)
					{
						gassociations = new ArrayList<Association>();
						gene2Associations.put(assoc.getDB_Object(), gassociations);
					...
					/* ADDED dbReference2Gene has a mapping from dbReference to gene names */
					//dbReference2gene.put(new Integer(assoc.getDbReference().hashCode()), assoc.getObjectSymbol());
				} catch (Exception ex)
				{
					bad++;
...
//			logger.info("A total of " + usedGoTerms.size() + " terms are directly associated."); 
	...
	private void importAffyFile(BufferedReader in, FileInputStream fis, HashSet<ByteString> names, TermContainer terms, IAssociationParserProgress progress) throws IOException
	{
		...
		if (!headerFailure)
		{
			...
			while ((line = in.readLine()) != null)
			{
				 ...
				 * variables */
				ByteString probeid = null,swiss = null, omim=null;
	...
	/** ADDED*/
/*	public HashMap<Integer, ByteString> getDbRerefence2gene()
	{
		return dbReference2gene;
	}*/
