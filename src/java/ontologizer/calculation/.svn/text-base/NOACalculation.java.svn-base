 ...
 */
public class NOACalculation extends AbstractHypergeometricCalculation
{
	...
	{
		EnrichedGOTermsResult studySetResult = new EnrichedGOTermsResult(graph, goAssociations, studySet, populationSet.getSize());
		...
		class SinglePValuesCalculation implements IPValueCalculation
		{
			...
			private PValue [] calculatePValues(StudySet studySet)
			{
				...
				for(TermID term : populationTermEnumerator)
				{
					...
					int popGeneCount = populationSet.getSize();
					int studyGeneCount = studySet.getSize();
					...
					myP.annotatedPopulationGenes = goidAnnotatedPopGeneCount;
                                        myP.omimlist = studyTermEnumerator.getAnnotatedGenes(term).totalAnnotated;
			...
			{
				return observedStudySet.getSize();
			}

			public PValue[] calculateRandomPValues()
			{
				return calculatePValues(populationSet.generateRandomStudySet(observedStudySet.getSize()));
			}
		};
