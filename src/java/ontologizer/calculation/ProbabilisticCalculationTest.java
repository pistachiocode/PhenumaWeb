...
public class ProbabilisticCalculationTest extends TestCase {
	...
	public void testCalculateStudySet()
	{
		...
		for (int i=0;i<10000;i++)
			pop.addElement(new ByteString("gene"+i), "");
		...
		/* for C2 */
		for (int i=0;i<10;i++)
			assocContainer.addAssociation(new Association(new ByteString("gene"+i),2), null);

		/* for C3 */
		for (int i=10;i<20;i++)
			assocContainer.addAssociation(new Association(new ByteString("gene"+i),3), null);

		/* for C4 */
		for (int i=20;i<100;i++)
			assocContainer.addAssociation(new Association(new ByteString("gene"+i),4), null);
		...
		/* 9 of 10 in C2 */
		for (int i=0;i<9;i++)
			study.addElement(new ByteString("gene"+i), "");
