...
public class Bayes2GOCalculation implements ICalculation
{
			...
			StudySet studySet)
	{
		...
		if (studySet.getSize() == 0)
			return new EnrichedGOTermsResult(graph,goAssociations,studySet,populationSet.getSize());
		...
		Bayes2GOEnrichedGOTermsResult result = new Bayes2GOEnrichedGOTermsResult(graph,goAssociations,studySet,populationSet.getSize());
	...
	private static void createInternalOntology(long seed)
	{
		...
		for (int i=1;i<=500;i++)
		{
			...
			for (int j=0;j<numTerms;j++)
			{
				...
				assoc.addAssociation(new Association(new ByteString(itemName),tid), null);
			}
		}
	}
...
//		Random rnd = new Random(11); /* Produces a set */
		...
		for (ByteString gene : assoc.getAllAnnotatedPP())
			allGenes.addElement(gene, "");
...
//		graph.setRelevantSubontology("biological_process");
		...
		for (TermID t : wantedActiveTerms.keySet())
		{
			System.out.println(t.toString() + " genes=" + wantedActiveTerm2StudySet.get(t).getSize() + " beta=" + wantedActiveTerms.get(t));
		...
		System.out.println("Number of genes in study set " + newStudyGenes.getSize());
		
		double alphaStudySet = B2GTestParameter.ALPHA;
		double betaStudySet = B2GTestParameter.BETA;

		int tp = newStudyGenes.getSize();
		int tn = allGenes.getSize();
		...
		System.out.println("Study set has a total of " +  newStudyGenes.getSize() + " genes");
