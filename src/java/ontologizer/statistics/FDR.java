						   ...
						   implements IResampling
{
	...
	public PValue[] adjustPValues(IPValueCalculation pvalues)
	{
                PrintWriter pw = null;
                try {
                    pw = new PrintWriter(new File("./resources/random_pvalues.txt"));
		...
		for (int b=0; b < numberOfResamplingSteps; b++)
		{
			...
			for (i=0;i<m;i++){
				pValues[b][i] = randomRawP[i].p;
                                pw.print(randomRawP[i].p + " ");
                        }
                        pw.print("\n");
			System.out.println("created " + (b+1) + " samples out of " + numberOfResamplingSteps + "\r");	
		}
		System.out.println("");
...
//			
			...
			double expectedRejections = ((double)totalRejects) / (numberOfResamplingSteps*m);
...
//			rawP[i].p_adjusted = fdr;
                        ...
                        rawP[i].p_adjusted = expectedRejections;
		}
