...
public class StudySetTest extends TestCase
{
	...
	public void testGenerateRandomStudySetInt()
	{
		...
		for (int i = 0;i<10000;i++)
			bigStudy.addElement(new ByteString("Gene"+i),"Description"+i);

		for (int i = 0; i<100;i++)
		{
			StudySet randomStudy = bigStudy.generateRandomStudySet(100);
			Assert.assertTrue(randomStudy.getSize() == 100);
		}
	}
	...
	public void testGenerateShuffledStudySets()
	{
		...
		for (int i = 0;i<200;i++)
			firstStudy.addElement(new ByteString("Gene"+i),"Description"+i);
		
		/* Generate the second study (which overlaps with 1st study
		 * in 100 genes and conists of 200 more genes) */
		for (int i = 100;i<400;i++)
			secondStudy.addElement(new ByteString("Gene"+i),"Description"+i);
		...
		for (int i=0;i<100;i++)
		{
			...
			Assert.assertTrue(shuffledArray[0].getSize() == 200);
			Assert.assertTrue(shuffledArray[1].getSize() == 300);
		}
	}
