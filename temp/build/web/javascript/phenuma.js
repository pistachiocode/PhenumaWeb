...
function passFilter(data, filter)
{
    ...
    if(found!=-1)
    {
        var x = data.substring(0, found);
        var y = data.substring(found, found+filter.length);
        var z = data.substring(found+filter.length, data.length);

        return x + "<span style='color:red;'>"+y+"</span>"+z;
    }
...
function go_to_page(page_num, table){
 ...
 /*   function showModalProcessing(text)
    {
        $.modal("<div><center><h3> "+text+" </h3> <img src='./resources/img/loader.GIF' style='width:40px; height: 40px'/> </center></div>")
    }*/
    
   /* function showModalSubnetwork(text, subnetwork)
    {
        $.when(showModalProcessing(text)).then(function () {
            showHideSubnetworks(subnetwork)
        });    
    }*/
